/* ============================================================================
   Ripple — Canvas UI (canvasui.dev), by David Haz. MIT + Commons Clause.
   https://canvasui.dev/docs/components/ripple

   The library ships TypeScript for React, Solid, Preact, Vue, Svelte and
   vanilla. This file is that vanilla factory with the types stripped, so a
   static page with no build step can use it. The logic is unchanged.

   Two rendering paths, and the page must work on both:
     - Chrome with html-in-canvas (behind a flag, or an origin trial token):
       the live DOM is drawn into a canvas and the shader refracts it, so the
       ripples bend the page itself.
     - Everywhere else: the same wave field renders as a transparent overlay of
       light glints. No refraction, no errors, nothing to apologise for.
   ============================================================================ */
...
export function createRipple(elements, options = {}) {
  ...
  const sourceCtx = source ? source.getContext("2d") : null;
  ...
  function spawnAmbient() {
    ...
    if (w < 10 || h < 10) return;
    splash(w * (0.15 + Math.random() * 0.7), h * (0.15 + Math.random() * 0.7),
           0.6 + Math.random() * 0.5);
  }
