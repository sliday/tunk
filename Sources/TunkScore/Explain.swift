...
enum Explainer {
    static func trace(session: Session, config: DetectorConfig) throws -> String {
        ...
        let score = try SessionScorer.score(session: session, replay: replay)
        ...
        if score.groups.isEmpty {
            ...
            for g in score.groups {
                ...
                out += "(intent `\(g.intent)`, confidence `\(g.confidence)`)\n\n"
                if let f = g.firstOnsetNs {
                    out += "- labelled first onset  \(at(f))\n"
                }
                out += "- labelled second onset \(at(g.secondOnsetNs))\n"
                out += "- candidate triggers inside ±\(Int(Double(Scoring.matchWindowNs) / 1e6)) ms: \(g.candidateTriggers)\n"
        ...
        out += "\n## Summary\n\n"
        out += "- \(score.detectedGroups)/\(score.doubleGroups) double-tap groups detected"
        if score.ambiguousGroups > 0 { out += ", \(score.ambiguousGroups) ambiguous (more than one trigger in the window)" }
        out += "\n- \(score.triggerCount) triggers, \(score.falsePositives) false positives\n"
    ...
    private static func reason(group g: GroupOutcome, onsets: [OnsetEvent],
        ...
        let near = onsets.filter { abs($0.tNs - g.secondOnsetNs) <= window * 2 }
        let thr = config.effectiveThreshold
        ...
        case .ambiguous:
            return "\(g.candidateTriggers) triggers landed inside the match window. FORMAT.md requires "
                + "exactly one, so this group is not detected and the surplus triggers count as false positives."
        case .missed:
            if near.isEmpty {
                if let p = probe.peak(around: g.secondOnsetNs, half: window) {
            ...
            if clear.count < 2 {
                return "only \(clear.count) ungated onset near this label; a double-tap needs two. "
